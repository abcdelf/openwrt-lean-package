...
if [ -s "/tmp/china_ssr_tmp.txt" ];then
  if ( ! cmp -s /tmp/china_ssr_tmp.txt /var/china_ssr.txt );then
    mv /tmp/china_ssr_tmp.txt /var/china_ssr.txt
  fi
fi
...
if [ -s "/tmp/gfwnew.txt" ];then
  if ( ! cmp -s /tmp/gfwnew.txt /var/dnsmasq.ssr/gfw_list.conf );then
    mv /tmp/gfwnew.txt /var/dnsmasq.ssr/gfw_list.conf
    echo "copy"
  fi
fi
