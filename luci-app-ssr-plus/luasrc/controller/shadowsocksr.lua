...
if set == "gfw_data" then
 ...
 if sret== 0 then
  ...
  if tonumber(icount)>1000 then
    ...
    oldcount=luci.sys.exec("cat /var/dnsmasq.ssr/gfw_list.conf | wc -l")
    luci.sys.exec("cp -f /tmp/gfwnew.txt /var/dnsmasq.ssr/gfw_list.conf")
...
elseif set == "ip_data" then
 refresh_cmd="wget -O- 'http://ftp.apnic.net/apnic/stats/apnic/delegated-apnic-latest'  2>/dev/null| awk -F\\| '/CN\\|ipv4/ { printf(\"%s/%d\\n\", $4, 32-log($5)/log(2)) }' > /tmp/china_ssr_tmp.txt"
 sret=luci.sys.call(refresh_cmd)
 icount = luci.sys.exec("cat /tmp/china_ssr_tmp.txt | wc -l")
 if  sret== 0 and tonumber(icount)>1000 then
  if nixio.fs.access("/etc/china_ssr.txt") then
   else
    luci.sys.exec("cp -f /tmp/china_ssr_tmp.txt /etc/china_ssr.txt")
    oldcount=0
  end
  -- oldcount=luci.sys.exec("cat /var/china_ssr.txt | wc -l")
  oldcount=0
  luci.sys.exec("cp -f /tmp/china_ssr_tmp.txt /var/china_ssr.txt")
  retstring=tostring(math.ceil(tonumber(icount)))
 else
  retstring ="-1"
 end
 luci.sys.exec("rm -f /tmp/china_ssr_tmp.txt ")
else
 ...
 if sret== 0 then
  if need_process == 1 then
    ...
    luci.sys.exec("rm -f /tmp/adnew.conf ")
  end
  ...
  if tonumber(icount)>1000 then
   ...
   oldcount=luci.sys.exec("cat /var/dnsmasq.ssr/ad.conf | wc -l")
   luci.sys.exec("cp -f /tmp/ad.conf /var/dnsmasq.ssr/ad.conf")
   retstring=tostring(math.ceil(tonumber(icount)))
   if oldcount==0 then
